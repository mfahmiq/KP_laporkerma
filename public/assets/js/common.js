/*
 *  Fungsi umum untuk membantu algoritma
 *
 */
 
function AddWhiteSpace(n,text){
	var x 	= parseInt(n)*10;
	text 	= new Array(x).join('&nbsp;') + text;
    return text;
};
...
function country_codes(search, col="alpha2") {
    let codes = [
    ...
    if(search!=undefined) {
        for(i in codes) {
            if(col=="alpha2") {
                if(codes[i].alpha2==search) return codes[i]; 
            }
            else if(col=="alpha3") {
                if(codes[i].alpha3==search) return codes[i]; 
            }
            else {
                if(codes[i].name==search) return codes[i]; 
            }
        }
        return false;
    }
